...
export interface BeadColor {
  ...
  g: number;
  b: number;
  brand: 'MARD' | 'DODO' | 'HAMA' | 'ARTKAL';
  category: 'basic' | 'fluorescent' | 'pearl' | 'glitter' | 'pastel' | 'metallic';
}
...
// 获取所有颜色
export const getAllColors = (): BeadColor[] => mardColors;

// 按品牌获取颜色
export const getColorsByBrand = (brand: BeadColor['brand']): BeadColor[] => 
  mardColors.filter(c => c.brand === brand);

// 按分类获取颜色
export const getColorsByCategory = (category: BeadColor['category']): BeadColor[] =>
  mardColors.filter(c => c.category === category);

// 通过色号查找颜色
export const findColorByCode = (code: string): BeadColor | undefined =>
  mardColors.find(c => c.code === code);
