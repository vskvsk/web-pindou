...
export function ciede2000(
  ...
  if (c1Prime * c2Prime === 0) {
    ...
    if (Math.abs(h1Prime - h2Prime) > 180) {
      if (hBarPrime < 180) hBarPrime += 180;
      else hBarPrime -= 180;
    }
  }
...
// 查找最接近的颜色（CIEDE2000）
export function findClosestColor<T extends { r: number; g: number; b: number; code: string }>(
  ...
  for (const color of colorLibrary) {
    ...
    const distance = ciede2000(
      ...
      colorLab.l, colorLab.a, colorLab.b
    );
...
// 快速查找（欧几里得距离）
export function findClosestColorFast<T extends { r: number; g: number; b: number; code: string }>(
  r: number, g: number, b: number,
  colorLibrary: T[]
): { color: T; distance: number } {
  let closestColor = colorLibrary[0];
...
export class ColorLUT {
  private cache = new Map<string, { code: string; hex: string }>();
  private colorLibrary: { r: number; g: number; b: number; code: string; hex: string }[];

  constructor(colorLibrary: { r: number; g: number; b: number; code: string; hex: string }[]) {
    this.colorLibrary = colorLibrary;
  }

  lookup(r: number, g: number, b: number, useCIEDE2000: boolean = true): { code: string; hex: string } {
    // 量化到 8 位以减少缓存大小
